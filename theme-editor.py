...
def refresh_theme():
    ...
    # Also render dynamic_text which is new in theme.yaml
    if 'dynamic_text' in config.THEME_DATA:
        # We need a way to render dynamic text in this simulator.
        # library/stats.py doesn't have a specific function for "render all dynamic text".
        # It usually runs in a loop in main.py.
        # For the editor, maybe we can't easily force it without modifying library/stats.py 
        # or implementing a renderer here.
        # BUT, the user asked to "Ensure dynamic_text support works".
        # Let's check if we can import the DynamicText class and trigger an update.
        try:
            from library.ui_components.dynamic_text import DynamicText
            from library.ui_renderer import UiRenderer
            
            renderer = UiRenderer(theme_folder=config.THEME_DATA['PATH'], 
                                 display_width=display.lcd.get_width(), 
                                 display_height=display.lcd.get_height())
            
            dt_config = config.THEME_DATA['dynamic_text']
            for key, settings in dt_config.items():
                if key == "INTERVAL": continue
                if isinstance(settings, dict) and settings.get("SHOW", False):
                    # Create a temporary DynamicText object just to render it once? 
                    # Actually, DynamicText is designed to be persistent.
                    # For a static preview, we might just want to render it using the renderer if possible,
                    # or instantiate it.
                    dt = DynamicText(key, settings, renderer)
                    dt.update() # This draws to the screen
                    
        except ImportError:
            pass # Maybe classes haven't been created yet or path issues
        except Exception as e:
            logger.error(f"Error rendering dynamic text: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Simple window geometry without rulers
