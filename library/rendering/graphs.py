...
def draw_line_graph(
    ...
    draw = ImageDraw.Draw(canvas)
    if len(plots_x) > 1:
        draw.line(list(zip(plots_x, plots_y)), fill=line_color, width=line_width)
    ...
    if graph_axis:
        ...
        if axis_font:
            ...
            draw.text((2, 0 - top), max_text, font=axis_font, fill=axis_color)

            min_text = f"{int(min_value)}"
            _, _, right, bottom = axis_font.getbbox(min_text)
            draw.text((width - 1 - right, height - 2 - bottom), min_text, font=axis_font, fill=axis_color)
...
def draw_radial_progress_bar(
    ...
    if draw_bar_background:
        ...
        draw.arc([0, 0, diameter - 1, diameter - 1], s, e, fill=bar_background_color, width=bar_width)
    ...
    if angle_sep == 0:
        ...
        draw.arc([0, 0, diameter - 1, diameter - 1], s, e, fill=bar_color, width=bar_width)
    else:
        # Discontinued bar
        ...
        if (clockwise and e_rem > s_rem) or (not clockwise and s_rem < e_rem):
             draw.arc([0, 0, diameter - 1, diameter - 1], s_rem, e_rem, fill=bar_color, width=bar_width)
